...
### 八进制转义序列以反斜杠（\）开始，后跟一个或多个八进制数字（0-7）。例如，\141 表示八进制数 141，转换为十进制是 97，在ASCII码表中对应的字符是 'a'。
### 十六进制转义序列也以反斜杠（\）开始，后跟字符 x 或 X，然后是一个或多个十六进制数字（0-9, a-f, A-F）。例如，\x61 表示十六进制数 61，转换为十进制是 97，在ASCII码表中对应的字符也是 'a'。
### Unicode 在Python字符串中，\u 转义序列用于表示一个Unicode字符。Unicode是一种字符编码标准，它支持世界上绝大多数的书写系统。\u 后面必须跟四个十六进制数字，这四个数字表示Unicode字符的码点。
```Python
s1 = '\'hello, world!\''
s2 = '\n\\hello, world!\\\n'
print(s1, s2, end='')
